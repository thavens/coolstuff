...
class Epicycle(mn.VMobject):
    ...
    def draw_circles(self, alpha, whole_mobject=None, **kwargs):
        ...
        for speed, radius in speeds_and_radii:
            circ = mn.Circle(abs(radius), **kwargs).set_opacity(0.5)
...
class EpicCycle(mn.Scene):
    ...
    def construct(self):
        ...
        for num_coeffs in range(5, 45+1, 20):
            ...
            #self.play(mn.FadeIn(mn.VGroup(text)), run_time=0.75)

            self.play(ep.animate_circles(color=mn.BLUE, stroke_width=1), run_time=10, rate_func=mn.linear)

            self.play(mn.FadeOut(mn.VGroup(ep)))